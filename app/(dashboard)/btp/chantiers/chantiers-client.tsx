...
export function ChantiersClient({ items: initialItems, description, canCreate }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && (
        <Card>
          <CardHeader><CardTitle>Nouveau chantier</CardTitle></CardHeader>
          <CardContent>
            ...
            <form action={handleCreate} className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2"><Label>Localisation</Label><Input name="location" /></div>
              <div className="space-y-2"><Label>Budget (GNF)</Label><Input name="budget" type="number" min="0" /></div>
              <div className="space-y-2"><Label>Avancement physique (%)</Label><Input name="physical_progress" type="number" min="0" max="100" defaultValue="0" /></div>
              <div className="space-y-2"><Label>Avancement financier (%)</Label><Input name="financial_progress" type="number" min="0" max="100" defaultValue="0" /></div>
      ...
      <div className="relative max-w-md">
        ...
        <Input placeholder="Rechercher dans chantiers..." className="pl-9" value={query} onChange={(e) => setQuery(e.target.value)} />
      </div>
      ...
      {items.length > 0 ? (
        ...
        <Card className="border-dashed">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-muted-foreground">Aucun chantier. Cliquez sur Ajouter pour en créer un.</p>
          </CardContent>
        </Card>
      )}
    </div>
