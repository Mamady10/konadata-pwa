...
export async function POST(request: NextRequest) {
  ...
  if (!secret) {
    return NextResponse.json(
      { success: false, error: 'Webhook non configuré (ORANGE_MONEY_WEBHOOK_SECRET)' },
      { status: 503 }
    );
  }
  ...
  const { data, error } = await supabase.rpc('process_billing_payment_webhook', {
  ...
  if (error) {
    console.error('[webhook/orange-money]', error.message);
    return NextResponse.json(
