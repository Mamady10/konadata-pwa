-- Candidat : plusieurs établissements, pas de blocage « déjà rattaché ailleurs ».
...
BEGIN
  ...
  IF v_pending_exists THEN
    RETURN jsonb_build_object('error', 'Vous avez déjà une demande en attente pour cet établissement.');
  END IF;
