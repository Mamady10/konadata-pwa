-- Codes de réinscription permanents (réutilisables chaque année scolaire)

CREATE UNIQUE INDEX IF NOT EXISTS school_reenrollment_codes_org_student_unique
  ON school_reenrollment_codes(organization_id, student_id)
  WHERE student_id IS NOT NULL;

COMMENT ON COLUMN school_reenrollment_codes.academic_year IS
  'NULL = code permanent multi-années ; sinon valable pour une année précise uniquement.';
...
DECLARE
  ...
  v_code_id UUID;
  v_code_permanent BOOLEAN := false;
...
BEGIN
  ...
  IF p_request_type = 'reenrollment' THEN
    ...
    SELECT id, (academic_year IS NULL) INTO v_code_id, v_code_permanent
    ...
    WHERE organization_id = p_org_id
      ...
      AND (
        ...
        OR (used_at IS NULL AND academic_year = p_academic_year)
      )
    LIMIT 1;
  ...
  IF v_code_id IS NOT NULL AND v_enrollment_id IS NOT NULL THEN
    IF v_code_permanent THEN
      UPDATE school_reenrollment_codes
      SET used_by_enrollment_id = v_enrollment_id
      WHERE id = v_code_id;
    ELSE
      UPDATE school_reenrollment_codes
