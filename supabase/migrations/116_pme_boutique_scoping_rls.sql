...
-- Un gérant (pme_staff) ne peut lire/écrire QUE les lignes des
-- boutiques qui lui sont assignées. Les directeurs (dont le
-- comptable) gardent l'accès complet.
-- PRÉREQUIS : migrations 114 et 115 déjà appliquées.
-- Idempotent : ré-exécutable.
-- ============================================================

-- Directeur PME = accès complet (inclut le comptable, contrairement à is_org_admin()).
...
-- Le gérant est-il assigné à cette boutique ? (directeur = toujours vrai)
...
-- ─── pme_boutiques : le gérant ne voit que ses boutiques ─────────
...
-- ─── Opérations rattachables à une boutique ──────────────────────
-- Règle : directeur = tout ; gérant = uniquement une boutique assignée
-- (les lignes sans boutique restent réservées aux directeurs).
