-- ============================================================
-- KonaData v2 — Codes d'accès organisation (invitations)
-- Max 2 responsables (org_admin + deputy_director) par entité
-- ============================================================

CREATE TABLE organization_access_codes (
...
CREATE UNIQUE INDEX idx_access_codes_code_upper ON organization_access_codes (upper(trim(code)));
CREATE INDEX idx_access_codes_org ON organization_access_codes (organization_id, is_active);

COMMENT ON TABLE organization_access_codes IS
  'Codes d''invitation générés par les responsables (max 2/org) pour rattacher un utilisateur à l''entité.';

-- ─── Helpers responsables & émission de codes ────────────────────
...
-- ─── Stats plateforme (platform_admin) ───────────────────────────
...
BEGIN
  ...
  RETURN jsonb_build_object(
    'organisations', (SELECT COUNT(*) FROM organizations WHERE is_active = true),
    'utilisateurs', (SELECT COUNT(*) FROM profiles WHERE is_active = true),
    'documents', (SELECT COUNT(*) FROM documents),
    'etudiants', (SELECT COUNT(*) FROM school_students),
    'projets_ong', (SELECT COUNT(*) FROM ngo_projects),
    'chantiers_btp', (SELECT COUNT(*) FROM btp_sites),
    'orgs_by_type', (
      SELECT COALESCE(jsonb_agg(jsonb_build_object('type', t.label, 'count', t.c)), '[]'::jsonb)
      FROM (
        SELECT CASE type
          WHEN 'school' THEN 'Établissements'
          WHEN 'ngo' THEN 'ONG'
          WHEN 'btp' THEN 'BTP'
          ELSE type::text
        END AS label,
