...
-- Exécuter après 063-F-ngo-survey-ceo-reprice-ONLY.sql
-- ============================================================
...
BEGIN
  ...
  FROM (
    SELECT 'phone' AS match_type, participant_phone_hash AS match_key,
      jsonb_agg(jsonb_build_object('id', id, 'created_at', created_at, 'locality', locality, 'answers', answers) ORDER BY created_at ASC) AS members
    FROM ngo_survey_responses
    WHERE survey_id = p_survey_id AND excluded_at IS NULL AND participant_phone_hash IS NOT NULL
    GROUP BY participant_phone_hash HAVING count(*) > 1
    UNION ALL
    SELECT 'device' AS match_type, device_hash AS match_key,
      jsonb_agg(jsonb_build_object('id', id, 'created_at', created_at, 'locality', locality, 'answers', answers) ORDER BY created_at ASC) AS members
    FROM ngo_survey_responses
    WHERE survey_id = p_survey_id AND excluded_at IS NULL AND device_hash IS NOT NULL
    GROUP BY device_hash HAVING count(*) > 1
    UNION ALL
    SELECT 'same_answer_locality' AS match_type,
  ...
  RETURN jsonb_build_object('groups', v_groups, 'group_count', jsonb_array_length(v_groups));
END;
$$;
...
BEGIN
  ...
  FOR i IN 0 .. COALESCE(jsonb_array_length(v_dup->'groups'), 0) - 1 LOOP
    ...
    FOR j IN 1 .. jsonb_array_length(v_members) - 1 LOOP
      ...
      IF EXISTS (SELECT 1 FROM ngo_survey_responses WHERE id = (v_member->>'id')::UUID AND excluded_at IS NULL) THEN
        UPDATE ngo_survey_responses SET
          excluded_at = now(), excluded_by = auth.uid(),
...
BEGIN
  ...
  FROM (
    SELECT to_char(date_trunc('day', created_at AT TIME ZONE 'Africa/Conakry'), 'YYYY-MM-DD') AS day, count(*)::INTEGER AS count
    FROM ngo_survey_responses WHERE survey_id = p_survey_id AND excluded_at IS NULL
  ...
  FROM (
    SELECT COALESCE(answers->>v_first_qid, '—') AS choice,
      COALESCE(NULLIF(trim(locality), ''), 'Non renseigné') AS locality, count(*)::INTEGER AS count
    FROM ngo_survey_responses WHERE survey_id = p_survey_id AND excluded_at IS NULL
  ...
  FROM (
    SELECT id, latitude::FLOAT AS lat, longitude::FLOAT AS lng,
      ...
      COALESCE(answers->>v_first_qid, '—') AS choice, created_at
    FROM ngo_survey_responses
    WHERE survey_id = p_survey_id AND excluded_at IS NULL
  ...
  SELECT jsonb_build_object(
    ...
    'total', count(*),
    'alerts', (SELECT count(*)::INTEGER FROM ngo_survey_security_alerts WHERE survey_id = p_survey_id AND acknowledged_at IS NULL),
    'duplicate_groups', COALESCE((v_dup->>'group_count')::INTEGER, 0)
  ) INTO v_quality FROM ngo_survey_responses WHERE survey_id = p_survey_id;

  SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY t.created_at DESC), '[]'::jsonb) INTO v_responses
  FROM (
    SELECT id, COALESCE(NULLIF(trim(locality), ''), '—') AS locality,
      COALESCE(answers->>v_first_qid, '—') AS answer, created_at,
      excluded_at IS NOT NULL AS is_excluded, exclusion_reason,
    ...
    FROM ngo_survey_responses WHERE survey_id = p_survey_id ORDER BY created_at DESC LIMIT 200
  ) t;

  RETURN jsonb_build_object(
    'survey_id', p_survey_id, 'title', v_survey.title, 'status', v_survey.status,
    'region', v_survey.region, 'collection_mode', v_survey.collection_mode,
    'starts_at', v_survey.starts_at, 'ends_at', v_survey.ends_at,
    'stats', v_stats, 'by_day', v_by_day, 'cross_tab', v_cross_tab,
    'map_points', v_map_points, 'quality', v_quality, 'responses', v_responses,
