-- 068 — Authentification compte par téléphone (OTP)
-- Exécuter dans Supabase SQL Editor si la migration 068 n'est pas encore appliquée.
...
CREATE INDEX IF NOT EXISTS idx_auth_phone_otp_phone_hash ON auth_phone_otp_challenges (phone_hash, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_auth_phone_otp_expires ON auth_phone_otp_challenges (expires_at);
...
DROP POLICY IF EXISTS auth_phone_otp_service ON auth_phone_otp_challenges;
