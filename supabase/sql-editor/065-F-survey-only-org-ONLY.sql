...
-- Exécuter après 064-F-ngo-survey-analytics-ONLY.sql
-- ============================================================
...
BEGIN
  ...
  VALUES (
    trim(p_name), 'ngo', p_email, p_phone, 'active',
    jsonb_build_object(
      'onboarding', jsonb_build_object(
        'intent', 'survey_only', 'declared_city', p_declared_city,
        'contact_name', p_contact_name, 'contact_title', p_contact_title,
        'application_profile', v_profile, 'submitted_at', now()
      ),
      'platform_billing_mode', 'survey_only',
      'ngo_surveys', jsonb_build_object('enabled', true, 'require_survey_payment', true, 'max_active_surveys', 5)
    )
  )
  RETURNING id INTO v_org_id;

  INSERT INTO organization_billing_offers (
    organization_id, status, billing_period, activation_amount_gnf, monthly_base_gnf,
    declared_city, declared_phone, application_profile, ceo_notes
  ) VALUES (
    v_org_id, 'paid', 'monthly', 0, 0, p_declared_city, p_phone, v_profile,
...
AS $$
  SELECT COALESCE(
    (SELECT settings->'onboarding'->>'intent' = 'survey_only' FROM organizations WHERE id = p_org_id),
    false
  );
$$;
...
BEGIN
  ...
  IF organization_is_survey_only(p_org_id) THEN RETURN true; END IF;
  IF v_org.billing_status IN ('pending_payment', 'pending_renewal', 'suspended') THEN RETURN false; END IF;
  IF v_org.type = 'school' THEN
    ...
    INTO v_org.billing_status, v_until FROM organizations WHERE id = p_org_id;
  ...
  IF v_sub.status IN ('trialing', 'active') AND v_sub.current_period_end >= now() THEN RETURN true; END IF;
  IF v_sub.status = 'past_due' AND v_sub.grace_until IS NOT NULL AND v_sub.grace_until >= now() THEN RETURN true; END IF;
  RETURN false;
END;
$$;
