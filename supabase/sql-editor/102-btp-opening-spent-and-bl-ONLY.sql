-- Exécuter après 101-btp-stock-movements-ONLY.sql
...
WHERE opening_spent = 0
  ...
  AND NOT EXISTS (SELECT 1 FROM btp_site_expenses e WHERE e.site_id = s.id)
  AND NOT EXISTS (SELECT 1 FROM btp_labor_entries l WHERE l.site_id = s.id);
